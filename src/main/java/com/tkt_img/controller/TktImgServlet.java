package com.tkt_img.controller;
...
//
        if ("insert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			req.setAttribute("errorMsgs", errorMsgs);
				...
				Integer tkt_no = Integer.valueOf(req.getParameter("tkt_no").trim());
				...
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("TktImgVO", tktImgVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/tkt_img/addtkt_img.jsp");
					failureView.forward(req, res);
					return;
				}
				
				/***************************2.開始新增資料***************************************/
				TktImgService tkt_imgSvc = new TktImgService();
				tktImgVO = tkt_imgSvc.addTktImg(tkt_no, null);
				
				/***************************3.新增完成,準備轉交(Send the Success view)***********/
				String url = "/emp/listAlltkt_img.jsp";
